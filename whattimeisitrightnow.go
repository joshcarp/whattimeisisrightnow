...
func ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	for _, timezone := range timezones{
		if strings.Contains(strings.ToLower(timezone),strings.ToLower(strings.ReplaceAll(command.Text, " ", "_"))){
			suc = true
	...
	_, err = http.Post(command.ResponseURL,
		"application/json", strings.NewReader(fmt.Sprintf(`{"channel":"%s", "text": "The time in %s is %s"}`, command.ChannelID, command.Text, t)))
}
